...
from tallykeep.services.profile_presets import ALL_FEATURE_FLAGS, DEFAULT_FLAG_VALUES
...
def test_patch_profile_rejects_preset_field(app_with_db) -> None:  # type: ignore[no-untyped-def]
    client, _ = app_with_db
...
def test_patch_profile_resets_to_defaults_with_empty_flags(app_with_db) -> None:  # type: ignore[no-untyped-def]
    client, _ = app_with_db
    # Set an override first
    client.patch("/api/v1/profile", json={"feature_flags": {"banking.rbf.enabled": True}})
    # Reset with empty dict
    resp = client.patch("/api/v1/profile", json={"feature_flags": {}})
    assert resp.status_code == 200
    assert resp.json()["feature_flags"] == {}
    # Flags endpoint should now return pure defaults
...
def test_feature_flags_default_matches_default_flag_values(
...
def test_feature_flags_apply_overrides_on_top_of_defaults(
    app_with_db,  # type: ignore[no-untyped-def]
) -> None:
    client, _ = app_with_db
    # Override one flag
    ...
    flags = client.get("/api/v1/feature-flags").json()["flags"]
    # Default has banking.rbf.enabled = False; override flips it
    assert flags["banking.rbf.enabled"] is True
    # Other default flags untouched
...
def test_feature_flags_includes_coin_selection_override(app_with_db) -> None:  # type: ignore[no-untyped-def]
    client, _ = app_with_db
    flags = client.get("/api/v1/feature-flags").json()["flags"]
    assert "banking.coin_selection_per_payment_override" in flags
